...
حدد رقم المادة التي تحتوي على معلومات تتعلق بالسؤال. أجب برقم المادة فقط:"""
    ...
    def _format_quranic_context(self, foundations: List[Dict[str, Any]]) -> str:
        ...
        context_parts.append("⚠️ **تعليمات التأصيل الشرعي الإلزامية:**")
        context_parts.append("- استخدم التفسير لبناء حجة قانونية قوية - لا تكتفِ بالذكر")
        context_parts.append("- اقتبس الآية الكريمة كاملة مع ذكر اسم السورة ورقم الآية")
        context_parts.append("- أظهر كيف يؤكد تفسير القرطبي على الحكم القانوني")
        context_parts.append("- أكد على أهمية هذا الأساس الشرعي في تقوية موقف الموكل")
        context_parts.append("- استخدم عبارات مثل: 'وفي هذا تأكيد شرعي قاطع...' أو 'ويؤكد الإمام القرطبي...'")
        context_parts.append("- اجعل التأصيل الشرعي جزءاً عضوياً من الحجة القانونية")
        context_parts.append("- المصدر: تفسير القرطبي المعتمد - الطبعة المحققة")
        context_parts.append("")
        ...
        for i, foundation in enumerate(foundations, 1):
            ...
            context_parts.append(f"📖 **المرجع الشرعي {i}: {verse_ref}**")
            context_parts.append(f"⚖️ **المبدأ الشرعي المؤكد للحكم القانوني**: {principle}")
            
            # Add detailed tafseer commentary with specific instructions
            if commentary:
                # Provide much more detailed tafseer content (up to 1000 characters)
                commentary_excerpt = commentary[:1000] + "..." if len(commentary) > 1000 else commentary
                context_parts.append(f"📜 **نص تفسير الإمام القرطبي الذي يجب الاستشهاد به**: {commentary_excerpt}")
                context_parts.append("🎯 **مطلوب**: استخدم هذا التفسير لإثبات أن الشريعة تؤكد الحق القانوني المطالب به")
                context_parts.append("🔥 **أهمية خاصة**: هذا التفسير يقوي الحجة القانونية بأساس شرعي راسخ")
    ...
    async def ask_question_with_context_streaming(
        ...
        try:
            ...
            # 🕌 STAGE 2.5: GET QURANIC FOUNDATIONS (DIRECT INTEGRATION)
            quranic_foundations = []
            print("🚨 DIRECT INTEGRATION: Starting direct Quranic search...")
            
            # DIRECT approach: Use the working QuranicFoundationStore directly
            try:
                from app.storage.quranic_foundation_store import QuranicFoundationStore
                from app.core.semantic_concepts import LegalConcept, ConceptType
                
                print("🚨 Initializing QuranicFoundationStore directly...")
                quranic_store = QuranicFoundationStore()
                await quranic_store.initialize()
                print("🚨 QuranicFoundationStore initialized successfully!")
                
                # Create employment concepts directly
                concepts = [
                    LegalConcept(
                        concept_id="basic_employment_dispute",
                        primary_concept="justice",
                ...
                print(f"🚨 Searching with {len(concepts)} concepts...")
                quranic_context = {"domain": "legal", "integration": True, "query": query}
                results = await quranic_store.semantic_search_foundations(concepts, quranic_context, limit=3)
                ...
                for i, result in enumerate(results):
                    ...
                    quranic_foundations.append({
                        "verse_reference": verse_ref,
                        "legal_principle": principle,
                        "commentary": result.chunk.content if hasattr(result, 'chunk') else result.content,
                        "confidence": getattr(result, 'similarity_score', 0.8),
                        "surah": metadata.get('surah', 'غير محدد'),
                        "modern_applications": metadata.get('modern_applications', [])
                    })
                
                print(f"🚨 SUCCESS: Found {len(quranic_foundations)} Quranic foundations!")
                logger.info(f"🕌 DIRECT INTEGRATION: Found {len(quranic_foundations)} Quranic foundations for legal strengthening")
                
            except Exception as e:
                print(f"🚨 ERROR: Direct integration failed: {e}")
                import traceback
                print(f"🚨 TRACEBACK: {traceback.format_exc()}")
                logger.error(f"Direct Quranic integration failed: {e}")
                quranic_foundations = []
