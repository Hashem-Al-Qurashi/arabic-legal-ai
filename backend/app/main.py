...
@app.post("/api/ask")
async def ask_legal_question_guest(
    query: str = Form(...),
    context: str = Form(None)
):
    """
    Guest access endpoint for direct legal questions.
    Now supports conversation context for better responses.
    """
    try:
        if not query or not query.strip():
            raise HTTPException(status_code=400, detail="السؤال مطلوب")
        
        print(f"🤖 Guest question: {query[:50]}...")
        ...
        if context:
            try:
                ...
                print("⚠️ Could not parse context, proceeding without it")
        
        # Use context-aware RAG if we have conversation history
        if conversation_history and len(conversation_history) > 0:
            ...
            answer = ask_question_with_context(query.strip(), backend_context)
        else:
        ...
        return {
            "id": f"guest_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
        ...
        print(f"❌ Guest question error: {e}")
