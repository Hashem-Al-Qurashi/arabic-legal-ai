...
class EliteLegalChunker:
    ...
    ARTICLE_PATTERNS = [
        ...
        r'(?:تعديلات\s*المادة\s*\n*)?المادة\s*(?:الأولى|الثانية|الثالثة|الرابعة|الخامسة|السادسة|السابعة|الثامنة|التاسعة|العاشرة|الحادية\s*عشrة|الثانية\s*عشرة|الثالثة\s*عشرة|الرابعة\s*عشرة|الخامسة\s*عشرة|السادسة\s*عشرة|السابعة\s*عشرة|الثامنة\s*عشرة|التاسعة\s*عشرة|العشرون|الحادية\s*والعشرون|الثانية\s*والعشرون|الثالثة\s*والعشرون|الرابعة\s*والعشرون|الخامسة\s*والعشرون)\s*:',
    ...
    def _standardize_article_reference(self, article_text: str) -> str:
        """Standardize all article formats to consistent Arabic format"""
        import re
        
        # Remove amendment prefixes
        cleaned = re.sub(r'تعديلات\s*المادة\s*\n*', '', article_text).strip()
        
        # Convert (digit) format to Arabic
        digit_match = re.search(r'المادة\s*\(\s*(\d+)\s*\)', cleaned)
        if digit_match:
            digit = int(digit_match.group(1))
            return self._convert_digit_to_arabic(digit)
        
        # Clean existing Arabic format
        arabic_match = re.search(r'المادة\s+([\u0600-\u06FF\s]+)', cleaned)
        if arabic_match:
            arabic_part = arabic_match.group(1).strip()
            arabic_part = re.sub(r'\s*:\s*$', '', arabic_part)
            return f"المادة {arabic_part}"
        
        return cleaned

    def _convert_digit_to_arabic(self, digit: int) -> str:
        """Convert digits to Arabic ordinals"""
        arabic_numbers = {
            1: 'الأولى', 2: 'الثانية', 3: 'الثالثة', 4: 'الرابعة', 5: 'الخامسة',
            6: 'السادسة', 7: 'السابعة', 8: 'الثامنة', 9: 'التاسعة', 10: 'العاشرة',
            11: 'الحادية عشرة', 12: 'الثانية عشرة', 13: 'الثالثة عشرة',
            14: 'الرابعة عشرة', 15: 'الخامسة عشرة', 20: 'العشرون',
            21: 'الحادية والعشرون', 22: 'الثانية والعشرون', 23: 'الثالثة والعشرون',
            24: 'الرابعة والعشرون', 25: 'الخامسة والعشرون'
        }
        
        return f"المادة {arabic_numbers.get(digit, f'({digit})')}"
    ...
    def _create_chunk_from_items(self, items: List[Dict[str, Any]], title: str, chunk_index: int, is_oversized: bool = False, inherited_context: Dict[str, str] = None) -> LegalChunk:
        ...
        metadata = {
            ...
            # Article metadata (FIXED WITH STANDARDIZATION)
        ...
        if metadata['articles']:
            metadata['articles'] = [self._standardize_article_reference(art) for art in metadata['articles']]
            # Update count after standardization
            metadata['unique_articles_count'] = len(metadata['articles'])
