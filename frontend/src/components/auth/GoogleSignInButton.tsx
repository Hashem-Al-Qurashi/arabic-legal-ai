...
const GoogleSignInButton: React.FC<GoogleSignInButtonProps> = ({ 
  ...
  const handleGoogleSignIn = async (response: any) => {
    ...
    try {
      ...
      console.log('👤 Google Sign-In: User data:', userData.email);

      // Update the auth context with the Google login
      ...
      // Navigate to dashboard or trigger a navigation if no custom handler
      if (!onSuccess) {
  ...
  const handleClick = () => {
    if (!googleLoaded || !window.google) {
      toast.error('Google Sign-In not loaded yet');
      return;
    }
  ...
  return (
    <button
      ...
      className={`
        ...
        ${(disabled || isLoading || !googleLoaded) ? 'opacity-50 cursor-not-allowed' : ''}
      `}
    >
      {isLoading ? (
        <>
